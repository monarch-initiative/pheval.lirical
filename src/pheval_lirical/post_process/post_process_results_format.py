...
from pheval.post_processing.post_processing import (
    PhEvalVariantResult, RankedPhEvalVariantResult, create_pheval_result,
    write_pheval_gene_result, write_pheval_variant_result, PhEvalGeneResult, RankedPhEvalGeneResult
)
...
def obtain_gene_symbol_from_identifier(gene_identifier: str, hgnc_data: dict, identifier: str) -> str:
...
class PhEvalGeneResultFromLirical:
    def __init__(self, lirical_result: pd.DataFrame, hgnc_data: dict, gene_identifier_updator: GeneIdentifierUpdater):
    ...
    def obtain_gene_symbol(self, result: pd.Series):
        ...
        return obtain_gene_symbol_from_identifier(gene_identifier.split(":")[1], self.hgnc_data, "entrez_id")
    ...
    def extract_pheval_requirements(self):
        ...
        for _index, result in self.lirical_result.iterrows():
            simplified_gene_results.append(PhEvalGeneResult(gene_symbol=self.obtain_gene_symbol(result),
        ...
        for _index, result in self.lirical_result.iterrows():
            ...
            for variant_result in variant_results:
                if variant_result != "":
                    ...
                    simplified_variant_results.append(PhEvalVariantResult(chromosome=self.obtain_chromosome(variant),
...
def create_standardised_results(
        results_dir: Path, output_dir: Path, sort_order: str
) -> None:
    ...
    for result in files_with_suffix(results_dir, ".tsv"):
        ...
        pheval_variant_result = create_variant_gene_result_from_lirical(
...
@click.option("--lirical-file", "-f", required=True, help="Path to Lirical results file.", type=Path)
@click.option("--output-dir", "-o", required=True, help="Path to output directory.", type=Path)
@click.option("--score-order", "-s", required=True, help="score order.", type=click.Choice(["ascending", "descending"]),
              default="descending", show_default=True)
def post_process(lirical_file: Path, output_dir, score_order):
    create_standardised_results(lirical_file, output_dir, score_order)
